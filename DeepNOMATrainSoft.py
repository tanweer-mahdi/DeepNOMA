    ...
    def __init__(self, structure):
        ...
        for i in structure:
            if i != 'output_layer':
                ...
                a,b = structure[i]
                self.hidden.append(nn.Linear(a,b))
                #self.hidden.append(nn.Softmax(a,b))
...
def custom_softmax(model_output, labels):
    temptensor = torch.mul(torch.log_softmax(model_output, dim=1), -1*labels/4)
    return torch.mean(temptensor.sum(dim=1))
    ...
    if fold == 0:
        for epoch in range(200):
            for i, val in enumerate(trainloader):
                ...
                yhat = model(inputs)
                # calculate loss
                loss = custom_softmax(yhat, targets)
                ...
                if i % 20 == 19:
                    ...
                    for j, batch in enumerate(testloader):
                        test, labels = batch
                        ypred = model(test)
                        runval = custom_softmax(ypred, labels)
