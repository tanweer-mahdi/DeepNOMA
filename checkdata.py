...
if codebook == 'sparse':
    phi = np.zeros((M, N))
    nz_entries = int(M/4)  # number of non-zero entries
...
for ii in range(ensemble):
    ...
    training_name = 'rtraining_set_' + str(ii) + '.npy'
    labels_name = 'rlabels_' + str(ii) + '.npy'
...
# saving dev data
with open('rdev_data.npy', 'wb') as file:
    np.save(file, dataset)

with open('rdev_labels.npy', 'wb') as file:
    np.save(file, labels)
